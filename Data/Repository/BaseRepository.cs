...
namespace Data.Repository
{
    public class BaseRepository<T> : IBaseRepository<T> where T : BaseEntity
    {
        ...
        public async Task<bool> DeleteAsync(int id)
        {
            try
            {
                T? result = await _dataset.AsNoTracking().SingleOrDefaultAsync(p => p.Id.Equals(id));
                if (result == null)
                {
                    return false;
                }
        ...
        public async Task<bool> ExistAsync(int id) => await _dataset.AnyAsync(p => p.Id.Equals(id));
        ...
        public async Task<IEnumerable<T>> SelectAsync(Func<T, object>? funcToOrderBy = null)
        {
            try
            {
                return funcToOrderBy != null
                    ? _dataset.OrderBy(funcToOrderBy).ToList()
